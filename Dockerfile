...
RUN sed -i 's/archive/in.archive/g' /etc/apt/sources.list
RUN apt-get update && apt-get --allow-unauthenticated install -y git zend-server-php-5.6 && /usr/local/zend/bin/zendctl.sh stop
